...
void apDC::SetBackground( const wxBrush &brush )
{
    if( dc )
...
void apDC::SetPen( const wxPen &pen )
{
    if( dc ) {
...
void apDC::SetBrush( const wxBrush &brush )
{
...
void apDC::SetTextForeground( const wxColour &colour )
{
...
void apDC::SetFont( const wxFont& font )
{
...
const wxPen& apDC::GetPen() const
{
    if( dc ) return dc->GetPen();
    return m_pen;
}

const wxBrush& apDC::GetBrush() const
{
    if( dc ) return dc->GetBrush();
    return m_brush;
}

const wxFont& apDC::GetFont() const
{
    if( dc ) return dc->GetFont();
    return m_font;
}

void apDC::GetSize( wxCoord *width, wxCoord *height ) const
{
    if( dc )
...
bool apDC::s_pushed_blend = false;
void apDC::SetGLStipple(int style, bool needs_blend)
{
#ifdef ocpnUSE_GL
...
void apDC::DrawLine( wxCoord x1, wxCoord y1, wxCoord x2, wxCoord y2, bool b_hiqual )
{
    if( dc )
...
void apDC::DrawLines( int n, wxPoint points[], wxCoord xoffset, wxCoord yoffset, bool b_hiqual )
{
    if( dc )
...
void apDC::StrokeLine( wxCoord x1, wxCoord y1, wxCoord x2, wxCoord y2 )
{
#if wxUSE_GRAPHICS_CONTEXT
    if( pgc ) {
...
void apDC::StrokeLines( int n, wxPoint *points) {
...
void apDC::DrawRectangle( wxCoord x, wxCoord y, wxCoord w, wxCoord h )
{
    if( dc )
...
void apDC::DrawRoundedRectangle( wxCoord x, wxCoord y, wxCoord w, wxCoord h, wxCoord r )
{
    if( dc )
...
void apDC::DrawCircle( wxCoord x, wxCoord y, wxCoord radius )
{
    DrawEllipse( x - radius, y - radius, 2 * radius, 2 * radius );
}

void apDC::StrokeCircle( wxCoord x, wxCoord y, wxCoord radius )
{
#if wxUSE_GRAPHICS_CONTEXT
    if( pgc ) {
...
void apDC::DrawEllipse( wxCoord x, wxCoord y, wxCoord width, wxCoord height )
{
    if( dc )
...
void apDC::DrawPolygon( int n, wxPoint points[], wxCoord xoffset, wxCoord yoffset )
{
    if( dc )
...
void apDC::StrokePolygon( int n, wxPoint points[], wxCoord xoffset, wxCoord yoffset )
{
#if wxUSE_GRAPHICS_CONTEXT
    if( pgc ) {
        wxGraphicsPath gpath = pgc->CreatePath();
...
void apDC::DrawBitmap( const wxBitmap &bitmap, wxCoord x, wxCoord y, bool usemask )
{
...
void apDC::DrawText( const wxString &text, wxCoord x, wxCoord y )
{
    if( dc )
...
void apDC::GetTextExtent( const wxString &string, wxCoord *w, wxCoord *h, wxCoord *descent,
...
void apDC::ResetBoundingBox()
{
    if( dc ) dc->ResetBoundingBox();
}

void apDC::CalcBoundingBox( wxCoord x, wxCoord y )
{
    if( dc ) dc->CalcBoundingBox( x, y );
}

bool apDC::ConfigurePen()
{
...
void apDC::GLDrawBlendData( wxCoord x, wxCoord y, wxCoord w, wxCoord h, int format,
