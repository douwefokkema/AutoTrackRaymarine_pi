...
PROD_REPO=${CLOUDSMITH_PROD_REPO:-'opencpn/autotrackraymarine-prod'}
BETA_REPO=${CLOUDSMITH_BETA_REPO:-'opencpn/autotrackraymarine-beta'}
ALPHA_REPO=${CLOUDSMITH_ALPHA_REPO:-'opencpn/autotrackraymarine-alpha'}
...
if [ -n "${OCPN_TARGET}" ]; then
    tarball_name=AutoTrackRaymarine_pi-1.1.1.2-${PKG_TARGET}-x86_64-${PKG_TARGET_VERSION}-${OCPN_TARGET}-tarball
else
    tarball_name=AutoTrackRaymarine_pi-1.1.1.2-${PKG_TARGET}-x86_64-${PKG_TARGET_VERSION}-tarball
fi
...
if [ "$BUILD_BRANCH_LOWER" = "master" ]; then
    ...
    if [ -n "$BUILD_TAG"  ]; then
        ...
        VERSION="1.1.1.2+${BUILD_ID}.${commit}"
...
# Only upload if the owner did the pull request. Non-owners probably don't have security to upload to the cloudsmith repository
    if [ "$CIRCLE_PROJECT_USERNAME" = "$CIRCLE_USERNAME" ]; then
        cloudsmith push raw --republish --no-wait-for-sync \
            --name AutoTrackRaymarine_pi-1.1.1.2-msvc-x86_64-10.0.22000-${OCPN_TARGET}-metadata \
            --version ${VERSION} \
            --summary "AutoTrackRaymarine opencpn plugin metadata for automatic installation" \
            $REPO $xml

        cloudsmith push raw --republish --no-wait-for-sync \
            --name $tarball_name \
            --version ${VERSION} \
            --summary "AutoTrackRaymarine opencpn plugin tarball for automatic installation" \
            $REPO $tarball

        if [ "${PKG_EXT}" != "gz" ] && [ "$pkg" != "" ]; then
            cloudsmith push raw --republish --no-wait-for-sync \
                --name opencpn-package-AutoTrackRaymarine-1.1.1.2-msvc-x86_64-10.0.22000-${OCPN_TARGET}.${PKG_EXT} \
                --version ${VERSION} \
                --summary "AutoTrackRaymarine .${PKG_EXT} installation package" \
                $REPO $pkg
        fi
    fi
fi
